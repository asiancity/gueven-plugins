...
function rs_column( $atts, $content = '', $id = '' ) {
  ...
  return '<div'. $id .' class="wpb_column col-'.sanitize_html_class($class_type).'-'. rs_get_bootstrap_col( $width ) . sanitize_html_classes($offset . $class .$align. $animation. $is_padding. $css_class).'"'.$animation_delay.'>'.do_shortcode( wp_kses_post($content)).'</div>';
}
